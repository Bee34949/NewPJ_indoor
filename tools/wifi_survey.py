# file: tools/wifi_survey.py
# v2.1 — แก้พาร์ส Windows ให้ทนภาษาไทย/โลคัลไลซ์ + คง probe/survey/build/locate
import argparse, json, os, sys, time, subprocess, platform, re, statistics, math
...
def norm_bssid(s: str)->str:
    ...
    s = re.sub(r'[^0-9a-f:]', '', s)
    return ':'.join([p.zfill(2) for p in s.split(':') if p][:6])
def median(vals): 
    try: return float(statistics.median(vals)) if vals else FILL_RSSI
    except statistics.StatisticsError: return FILL_RSSI
def mean(vals): return float(sum(vals)/len(vals)) if vals else FILL_RSSI

def _run(cmd: List[str], text=True):
    return subprocess.check_output(cmd, encoding="utf-8" if text else None, errors="ignore")

# -------- Windows: robust parser (locale-agnostic) --------
def scan_windows(debug_file: Optional[Path]=None)->List[Tuple[str,int,str]]:
    """
    Parse 'netsh wlan show networks mode=bssid' in ANY locale.
    Strategy:
      - Detect SSID header lines leniently (contains 'SSID' or Thai 'ชื่อเครือข่าย' etc.)
      - BSSID lines: capture MAC regardless of preceding label
      - Signal lines: capture first '<number> %' after a BSSID (before next BSSID/SSID)
    """
    # Use OEM codepage to avoid garbled Thai; fallback to cp1252
    out = subprocess.check_output(["netsh","wlan","show","networks","mode=bssid"])
    try:
        # try system OEM codepage
        oem = subprocess.check_output(["chcp"], shell=True).decode(errors="ignore")
        cp = re.search(r":\s*(\d+)", oem)
    ...
    is_ssid_line = lambda s: bool(re.search(r"\bSSID\b|\u0e0a\u0e37\u0e48\u0e2d\u0e40\u0e04\u0e23\u0e37\u0e2d\u0e02\u0e48\u0e32\u0e22", s, re.I))
    ...
    while i < len(lines):
        ...
        # Update SSID if line looks like an SSID header
        if is_ssid_line(ln):
            # try to get SSID after colon
            m = re.search(r":\s*(.+)$", ln)
            current_ssid = (m.group(1).strip() if m else None) or current_ssid
            i += 1
            continue
        ...
        if m_mac:
            ...
            # look ahead for a percentage line as signal
            rssi = None
            j = i + 1
            while j < len(lines):
                ln2 = lines[j]
                # break on next BSSID or SSID section
                if mac_re.search(ln2) or is_ssid_line(ln2):
                    break
                ...
                j += 1
            if rssi is None:
                # if no % found, skip this AP (unlikely)
                i += 1
                continue
            results.append((bssid, rssi, current_ssid or ""))
            i = j
            continue
...
def scan_linux(iface: Optional[str], debug_file: Optional[Path]=None)->List[Tuple[str,int,str]]:
    ...
    for ln in raw.splitlines():
        ...
        if ms and cur_b is not None:
            ...
            res.append((cur_b, cur_r if cur_r is not None else FILL_RSSI, cur_s))
...
def cmd_probe(args):
    ...
    scans = do_scan(args.iface, debug)
    if not scans:
        print("⚠️  ไม่พบ AP — ตรวจสิทธิ์/เปิด Wi-Fi/หลีกเลี่ยง WSL/VM และลอง --debug")
        return
...
def cmd_survey(args):
    ...
    for i in range(args.scans):
        ...
        s = do_scan(args.iface, dbg)
        sample = apply_filters(s, args.ssid_filter, bssid_whitelist)
    ...
    if not seen_any:
        print("❌ ไม่เห็น AP เลย → ไม่บันทึกจุดนี้ (ตรวจสิทธิ์/เปิด Wi-Fi/ลอง --debug)")
        return
    agg = { b:int(median(vs)) for b,vs in all_samples.items() }
    rec = {"id":args.id,"label":args.id,"floor":str(args.floor),"lon":float(args.lon),"lat":float(args.lat),"rssi":agg,
           "stats":{"samples":{b:{"n":len(v),"mean":mean(v),"median":median(v)} for b,v in all_samples.items()},"ts":now_iso()}}
...
def cmd_build(args):
    ...
    if not points: print("❌ ยังไม่มีจุดใน points.json"); return
    ap_set=set()
    for p in points:
    ...
    out={
        "dataset": args.dataset, "dim": M, "ap_dict": ap_idx, "weights_sqrt": sqrt_w,
    ...
    print(f"✅ signatures.json สร้างแล้ว (dim={M}, points={len(points)})")

def _vectorize(signatures, scan_map):
    ap_idx = signatures["ap_dict"]; w = signatures["weights_sqrt"]
    M = signatures["dim"]; X=[FILL_RSSI]*M
...
def cmd_locate(args):
    ...
    if not sig: print("❌ ไม่มี signatures.json — run build-dataset ก่อน"); return
    scans = do_scan(args.iface, Path(args.debug) if args.debug else None)
    if not scans: print("❌ สแกนไม่ได้ — probe เช็คก่อน"); return
    sample = apply_filters(scans, args.ssid_filter, None)
    if not sample: print("⚠️ ไม่ผ่าน filter → ได้ 0"); return
    X = _vectorize(sig, sample)
    aps = sig["ap_dict"]; W = sig["weights_sqrt"]
    def prescale(vec): return [ vec.get(b, FILL_RSSI)*W[j] for b,j in aps.items() ]
    cands=[]
    for p in sig["points"]:
        S = prescale(p["rssi"])
        d = _dist_l2(X,S)
    ...
    wsum=0; wx=0; wy=0
    for d,p in top:
        w = 1/(d+1e-6); wsum+=w; wx+=p["lon"]*w; wy+=p["lat"]*w
    est = {"lon": wx/wsum, "lat": wy/wsum, "floor": top[0][1]["floor"]}
    print("neighbors:")
    for d,p in top:
        print(f"  {p['id']} F{p['floor']}  d={d:.3f}  at ({p['lon']:.6f},{p['lat']:.6f})")
    print(f"→ estimate: ({est['lon']:.6f},{est['lat']:.6f})  floor F{est['floor']}")

# --------------- CLI ----------------
def main():
    ap = argparse.ArgumentParser(description="Wi-Fi Fingerprint Survey Tool")
    sub = ap.add_subparsers(dest="cmd", required=True)

    p = sub.add_parser("probe", help="สแกน 1 ครั้ง")
    p.add_argument("--iface"); p.add_argument("--debug", help="โฟลเดอร์บันทึก output ดิบ")
    p.set_defaults(func=cmd_probe)

    s = sub.add_parser("survey", help="เก็บที่จุดเดียวหลายสแกน")
    s.add_argument("--dataset", required=True); s.add_argument("--id", required=True)
    s.add_argument("--floor", required=True); s.add_argument("--lon", type=float, required=True)
    ...
    s.add_argument("--ssid-filter"); s.add_argument("--whitelist")
    s.add_argument("--iface"); s.add_argument("--debug")
    s.set_defaults(func=cmd_survey)

    b = sub.add_parser("build-dataset", help="รวม → signatures.json")
    b.add_argument("--dataset", required=True)
    b.set_defaults(func=cmd_build)

    l = sub.add_parser("locate", help="สแกนสดแล้วคำนวณตำแหน่ง")
